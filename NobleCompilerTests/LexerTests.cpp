...
TEST(Lexer, String)
{
    ...
    const std::vector<Token> tokens = lexer.Lex(testString);

    auto [type, firstCharacter, lastCharacter] = tokens[0];

    EXPECT_EQ(type, Token::Type::String);
    EXPECT_EQ(memcmp(firstCharacter, testString.c_str(), lastCharacter - firstCharacter), 0);
}
